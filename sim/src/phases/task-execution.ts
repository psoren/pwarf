...
export async function taskExecution(ctx: SimContext): Promise<void> {
  ...
  for (const dwarf of state.dwarves) {
    ...
    if (task.target_x !== null && task.target_y !== null && task.target_z !== null) {
      ...
      if (!atSite) {
        // Wander, socialize, and rest use cheap Manhattan movement instead of BFS.
        // These idle tasks always target open terrain (wander: random open tile;
        // socialize: other dwarf's position; rest: well/mushroom_garden).
        // Greedy Manhattan movement is sufficient and avoids BFS GC pressure.
        // Wander also accumulates work_progress while traveling.
        if (IDLE_TASK_TYPES_EXECUTION.has(task.task_type)) {
          if (task.task_type === 'wander') {
            task.work_progress += BASE_WORK_RATE;
            state.dirtyTaskIds.add(task.id);
          ...
          stepTowardWander(dwarf, task, ctx, occupiedTiles);
          continue;
        }
...
/**
 * Move a dwarf one step toward a wander task's target using cheap Manhattan navigation.
 * Wander targets are always on open terrain (generated by findWanderTarget which checks
 * walkability), so greedy Manhattan movement is correct and avoids the GC pressure of BFS.
 * The dwarf moves along whichever axis has greater remaining distance first; if that step
 * is occupied, tries the other axis; if both occupied, stays put (rare, non-blocking).
 */
function stepTowardWander(dwarf: Dwarf, task: Task, ctx: SimContext, occupiedTiles: Set<string>): void {
  if (task.target_x === null || task.target_y === null) return;

  const dx = task.target_x - dwarf.position_x;
  const dy = task.target_y - dwarf.position_y;
  if (dx === 0 && dy === 0) return;

  // Try the axis with greater distance first, then the other
  const steps: [number, number][] = [];
  if (Math.abs(dx) >= Math.abs(dy)) {
    steps.push([Math.sign(dx), 0], [0, Math.sign(dy)]);
  } else {
    steps.push([0, Math.sign(dy)], [Math.sign(dx), 0]);
  }

  for (const [sx, sy] of steps) {
    if (sx === 0 && sy === 0) continue;
    const nx = dwarf.position_x + sx;
    const ny = dwarf.position_y + sy;
    const key = `${nx},${ny},${dwarf.position_z}`;
    if (occupiedTiles.has(key)) continue;
  ...
  // Both axes blocked — stay put (wander progress still accumulates each tick)
}

/** Idle task types that should be cancelled (not re-queued) when movement fails. */
