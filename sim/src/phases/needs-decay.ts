 ...
 *
 * 4 needs: food, drink, sleep, morale (stored in need_social).
 * need_purpose and need_beauty are unused (always 0).
 *
 * Personality traits modulate decay rates:
 * - trait_extraversion scales morale decay (extraverts crave contact more urgently)
 */
export async function needsDecay(ctx: SimContext): Promise<void> {
  for (const dwarf of ctx.state.dwarves) {
    ...
    dwarf.need_social = Math.max(MIN_NEED, dwarf.need_social - MORALE_DECAY_PER_TICK * extraversionModifier);
