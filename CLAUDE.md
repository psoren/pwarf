...
- **No hardcoded strings for domain values.** Task types, tile types, item categories, item materials, and structure types must not appear as bare string literals in logic. Use the typed constants and sets in `shared/src/constants.ts` (e.g., `ADJACENT_TASK_TYPES`, `SKILLED_TASK_TYPES`, `FORAGEABLE_TILE_TYPES`, `TASK_WORKSHOP_MAP`, `AUTONOMOUS_TASK_TYPES`, `IDLE_TASK_TYPES`). Switch-case branches on `task.task_type` are acceptable since TypeScript narrows the type, but `new Set(['mine', 'build_wall', ...])` inline is not — extract it to a shared constant. Magic number thresholds for noise generation (e.g., `0.85` for flower spawning) should also be named constants.
