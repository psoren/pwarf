-- Caves table: tracks individual caves per civilization with terrain-derived properties.
-- Cave rows are pre-created at embark time with discovered=false and updated
-- when a dwarf completes a scout_cave task.

CREATE TABLE caves (
  id                uuid PRIMARY KEY DEFAULT uuid_generate_v4(),
  civilization_id   uuid NOT NULL REFERENCES civilizations(id) ON DELETE CASCADE,
  entrance_x        int NOT NULL CHECK (entrance_x BETWEEN 0 AND 511),
  entrance_y        int NOT NULL CHECK (entrance_y BETWEEN 0 AND 511),
  z_level           int NOT NULL CHECK (z_level BETWEEN -19 AND -1),
  name              text NOT NULL,
  discovered        boolean NOT NULL DEFAULT false,
  discovered_by     uuid REFERENCES dwarves(id) ON DELETE SET NULL,
  discovered_at     timestamptz,
  danger_level      int NOT NULL DEFAULT 0 CHECK (danger_level BETWEEN 0 AND 100),
  resource_type     text,
  created_at        timestamptz NOT NULL DEFAULT now(),
  UNIQUE (civilization_id, z_level),
  UNIQUE (civilization_id, entrance_x, entrance_y)
);

CREATE INDEX caves_civ_idx ON caves(civilization_id);

-- Row Level Security
ALTER TABLE caves ENABLE ROW LEVEL SECURITY;

CREATE POLICY "public caves readable"
  ON caves FOR SELECT USING (true);

CREATE POLICY "players manage own caves"
  ON caves FOR ALL USING (
    civilization_id IN (SELECT id FROM civilizations WHERE player_id = auth.uid())
  );

-- Add caves to flush_state RPC
...
BEGIN
  ...
  IF jsonb_array_length(p_caves) > 0 THEN
    INSERT INTO caves (id, civilization_id, entrance_x, entrance_y, z_level,
      name, discovered, discovered_by, discovered_at,
      danger_level, resource_type, created_at)
